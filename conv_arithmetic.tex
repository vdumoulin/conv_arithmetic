...
the drawing simple we represented a single input feature map, but it is not
...
kernel starts on the leftmost part of the input feature map and slides by steps
of one until its right side touches the right side of the input. The size of
the output will be equal to the number of steps we made, plus one, accounting
for the initial position of the kernel
(\autoref{fig:no_padding_no_strides_explained}). The same logic applies for the
height axis.
