...
Let's now consider what would happen if we wanted to invert the operation we
just described, i.e., to expand the $2 \times 2$ matrix $\mathbf{O}$ with a
convolutional kernel in order to obtain the $4 \times 4$ matrix $\mathbf{I}$.
...
the gradient of {\em some\/} convolution with respect to its input, which is in
practice how transposed convolutions are usually implemented.}

We defined so far how to invert an existing convolution by applying the
transpose of its matrix representation. It is trivial to note that if we are
only interested in the transposed convolution, we don't necessarly need to
