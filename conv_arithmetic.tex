...
the drawing simple we representrd a single input feature map, but it is not
uncommon to have multiple feature maps stacked one over the other.\footnote{
    An example of this is what was referred to earlier as {\em channels} for 
    images and sound clips.}. 
A {\em kernel} (shaded area, resulting in dark blue) of value
...
\noindent is being slided across the input feature map. At each location, the
product between each element of the kernel and the corresponding elements of
the overlapped input is computed and the results are summed up to obtain the
output in the current location. The procedure can be repeated using different
kernels to form as many output feature maps as desired 
(\autoref{fig:full_picture}).  The final output of these procedure is called 
...
\mbox{3-dimensional -- or,} equivalently each one of the feature maps will be
convolved with a distinct \mbox{kernel -- and} the resulting feature maps will
be summed up elementwise to produce the output feature map. 

The convolution presented so far and depicted in 
\autoref{fig:numerical_no_padding_no_strides} is an instance of a 2-D
convolution, but it can be generalized to N-D convolutions.  For instance, in a
3-D convolution, the kernel would be a {\em cuboid} and would slide across the
...
can be also viewed as how much of the output is retained. For instance, moving
...
easied by the fact that they don't interact across axes, i.e. the choice of
kernel size, stride and zero padding along axis $j$ only affects the output
size of axis $j$.

Because of this, in this chapter we will focus on the following simplified 
setting:
...
placements of the kernel on the input. Let's consider the width axis. Imagine
to superimpose the kernel on the upper left part of the input feature map and
then move it towards the right. The size of the output will be equal to the
number of steps we need to make the right side of the kernel touch the right
side of the image, plus one, which corresponds to the initial superimposition
of the kernel on the upper-left side
(\autoref{fig:no_padding_no_strides_explained}).  The same logic applies for
the height axis.

More formally, the following relashionship can be inferred:
