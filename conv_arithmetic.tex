...
\renewcommand{\abstractname}{Feedbacks}
\begin{abstract}
    Your feedback is welcome!  We did our best to be as precise, informative
    and up to the point as possible, but should there be anything you feel
    might be an error or could be rephrased to be more precise or
    comprehensible, please don't refrain from contacting us. Likewise, drop us
    a line if you think there is something that might fit this technical report
...
\begin{figure}[p]
    ...
    \caption{\label{fig:same_padding_no_strides} (Same padding, no strides) 
...
Having the output size be the same as the input size (i.e., $i = o$) can be a
desirable property:
...
effective input of size $i + 2p$, analogously to what was done for
...
input sizes these convolution share the same output size. While this doesn't
...
\begin{figure}[p]
    ...
    \caption{\label{fig:no_padding_strides} (No zero-padding, arbitrary 
        stripes) Convolving a $3 \times 3$ kernel over a $5 \times 5$ input 
...
In a neural network, pooling layers provide invariance to small translation of 
