...
For $\forall i$, $\forall k$, $s = 1$ and $p = 0$,
\begin{equation*}
    o = (i - k) + 1.
\end{equation*}
\end{relationship}
...
For $\forall i$, $\forall k$, $s = 1$ and $\forall p$,
\begin{equation*}
    o = (i - k) + 2p + 1.
\end{equation*}
\end{relationship}
...
For $p = k - 1$, $\forall i$, $\forall k$ and $s = 1$,
\begin{equation*}
\begin{split}
    o &= i + 2(k - 1) - (k - 1) \\
      &= i + (k - 1).
...
For $\forall i$, $\forall k$, $p = 0$ and $\forall s$,
\begin{equation*}
...
For $\forall i$, $\forall k$, $\forall p$ and $\forall s$,
\begin{equation*}
...
For $\forall i$, $\forall k$ and $\forall s$,
\begin{equation*}
    o = \left\lfloor \frac{i - k}{s} \right\rfloor + 1.
\end{equation*}
\end{relationship}
...
$\mathbf{C}$; in other words, the error is backpropagated by multiplying with
$\mathbf{C}^T$. This operation takes a 4-dimensional vector as input and
produces a 16-dimensional vector as output, and its connectivity pattern is
compatible with $\mathbf{C}$ by construction.

In other words, the kernel $\mathbf{w}$ defines the matrices $\mathbf{C}$ and
...
put it is to note that the kernel defines a convolution, but its
nature (convolution or transposed convolution) is determined by how the forward
...
A convolution described by $\forall k$, $s = 1$ and $p = 0$ has an associated
...
A convolution described by $\forall k$, $s = 1$ and $\forall p$ has an
...
padding, it is unsurprising that the transpose of a fully padded convolution is
a non-padded convolution:

\begin{relationship}\label{rel:full_padding_no_strides_transposed}
A convolution described by $\forall k$, $s = 1$ and $p = k - 1$ has an
...
is why transposed convolutions are sometimes called {\em fractionally strided
convolutions}.
...
slower pace than with unit strides.\footnote{Doing so is inefficient and
...
A convolution described by $\forall k$, $\forall s$ and $p = 0$ and whose input
...
A convolution described by $\forall k$, $\forall s$ and $\forall p$ and whose
...
The constrain on the size of the input $i$ can be relaxed by introducing
...
A convolution described by $\forall k$, $\forall s$ and $\forall p$ has an
